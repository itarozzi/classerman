...
# ------------------------------------
#cognome;nome;nome_utente;elenco classi [separati da spazio]
#====================================================================
set -u
...
do
  ...
  classi=`echo $riga|awk -F";" {'print $4}'`
	...
	if [ -z "$cognome" ]; then msg_params   $riga 1 1; ERR="1"; continue; fi
	if [ -z "$nome" ]; then msg_params      $riga 2 1; ERR="2"; continue; fi
	if [ -z "$username" ]; then msg_params  $riga 3 1; ERR="3"; continue; fi
	if [ -z "$classi" ]; then msg_params    $riga 4 1; ERR="4"; continue; fi
...
if [ $errors -eq 0 ]; then
  echo "----------------------------------------------------"
  echo "[SCR4]Script completato con successo!"
  echo "----------------------------------------------------"
else
  echo "----------------------------------------------------"
  echo "[SCR4]Script completato con errore!"
  echo "----------------------------------------------------"
  exit 200
